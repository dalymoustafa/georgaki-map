...
async function main() {
  ...
  const record = allRecords.find(r =>
    r.fields["Name"] && r.fields["Name"].toLowerCase().includes("georgaki")
  );

  if (!record) { console.error("Could not find Georgaki Law Firm!"); process.exit(1); }
...
<head>
  ...
  <title>Georgaki Law Firm — Office Locations</title>
...
<script>
  ...
  fetch("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson")
    ...
    .then(geojson => {
      ...
      if (OFFICES.length > 1) {
        map.fitBounds(OFFICES.map(o => [o.lat, o.lng]), { padding: [30, 30], maxZoom: 2 });
      } else {
        map.setView([20, -20], 2);
      }
    });
